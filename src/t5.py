...
class T5Attention(nn.Module):
    ...
    def forward(
        ...
        if prompt_kv is not None:
            ...
            prompt_seq_len = prompt_k.shape[2]
            key_length += prompt_seq_len
            # real_seq_length += prompt_k.shape[2]
        ...
        present_key_value_state = (key_states[:, :, prompt_seq_len:, :], value_states[:, :, prompt_seq_len:, :]) \
    ...
    def forward(
        ...
        if prompt_kv is not None:
            if self.is_decoder is True:
                prompt_len = int(prompt_kv.shape[-1] / 2)
                prompt_kv_1 = prompt_kv[:, :, :, :, 0:prompt_len]
                prompt_kv_2 = prompt_kv[:, :, :, :, prompt_len:]
            else:
                prompt_kv_1 = prompt_kv
        else:
            prompt_kv_1 = None
            prompt_kv_2 = None
