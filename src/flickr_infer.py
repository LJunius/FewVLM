...
def build_model():
    ...
    cfg.MODEL.WEIGHTS = str(Path.home().joinpath('faster_rcnn/faster_rcnn_from_caffe_attr.pkl'))
...
if __name__ == '__main__':
    ...
    args = parse_args()
    args.gpu = 0
    img_path = '/root/nlp_hw/test.jpg'
    im = cv2.imread(img_path)
    im_rgb = cv2.cvtColor(im, cv2.COLOR_BGR2RGB)
    instances, features = doit(im, detector)
    prefix = 'an image of'
    input_tokens = [prefix]
    input_text = ' '.join(input_tokens)
    tokenizer = FewVLMTokenizerFast.from_pretrained(
        args.backbone,
    ...
    vis_feats = features.unsqueeze(0)
    vis_pos = instances.pred_boxes.tensor.unsqueeze(0)
    trainer = Trainer(args, None, None, None, train=False)

    results = trainer.infer(input_ids, vis_feats, vis_pos)
    print(results)